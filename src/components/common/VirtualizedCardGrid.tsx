...
const VirtualizedCardGrid = <T,>({
  ...
  const columns = xlUp
    ? resolvedColumns.xl ?? resolvedColumns.lg ?? resolvedColumns.md ?? resolvedColumns.sm ?? resolvedColumns.xs
    : lgUp
      ? resolvedColumns.lg ?? resolvedColumns.md ?? resolvedColumns.sm ?? resolvedColumns.xs
      : mdUp
  ...
  useEffect(() => {
    ...
    const updateVisibleRange = () => {
      ...
      const firstVisibleRow = Math.max(0, Math.floor((viewportTop - containerTop) / rowStride) - overscanRows)
  ...
  }, [columns, gap, itemHeight, items.length, overscanRows, shouldVirtualize, totalRows])
  ...
  useEffect(() => {
    onVisibleRangeChange?.(shouldVirtualize ? visibleRange : { start: 0, end: items.length })
  }, [items.length, onVisibleRangeChange, shouldVirtualize, visibleRange])

  const startRow = shouldVirtualize ? Math.floor(visibleRange.start / columns) : 0
  const renderedRows = Math.ceil(visibleItems.length / columns)
  const totalContentHeight = totalRows * itemHeight + Math.max(0, totalRows - 1) * gap
  const renderedHeight = renderedRows * itemHeight + Math.max(0, renderedRows - 1) * gap
  const topSpacerHeight = shouldVirtualize ? startRow * (itemHeight + gap) : 0
    ...
    <Box ref={containerRef}>
      ...
      >
        {visibleItems.map((item, index) => {
          ...
          return (
            <Box key={getItemKey(item, actualIndex)} sx={{ minHeight: itemHeight }}>
              {renderItem(item, actualIndex)}
            </Box>
          )
        })}
      </Box>
      {bottomSpacerHeight > 0 ? <Box sx={{ height: bottomSpacerHeight }} /> : null}
    </Box>
  )
}
