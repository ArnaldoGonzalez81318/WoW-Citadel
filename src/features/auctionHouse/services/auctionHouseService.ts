...
import { AuctionHouseResponse, AuctionItemSummary, AuctionListing, LocalizedString } from "@/features/auctionHouse/types"
...
const fetchItemSummary = async (itemId: number): Promise<AuctionItemSummary | null> => {
  try {
    const response = await blizzardClient.get<ItemDetailResponse>(`/data/wow/item/${itemId}`, {
      namespace: STATIC_NAMESPACE,
    })

    return {
      id: response.id,
      href: safeText(response._links?.self?.href, `https://${env.region}.api.blizzard.com/data/wow/item/${itemId}`),
...
const fetchItemSummaryMap = async (itemIds: number[]): Promise<Map<number, AuctionItemSummary>> => {
  const uniqueIds = Array.from(new Set(itemIds.filter((id) => typeof id === "number" && id > 0)))

  if (uniqueIds.length === 0) {
    return new Map<number, AuctionItemSummary>()
  }

  const results = await Promise.all(uniqueIds.map((itemId) => fetchItemSummary(itemId)))
...
const normalizeAuctionResult = (
  ...
  return {
    ...
    href: itemSummary?.href ?? `https://${env.region}.api.blizzard.com/data/wow/item/${listing.item.id}`,
    summary: valueLabel,
...
export const fetchCommoditySnapshots = async (
  ...
  response.auctions.slice(0, MAX_COMMODITY_SCAN).forEach((listing) => {
    ...
    current.unit_price = Math.min(current.unit_price ?? Number.MAX_SAFE_INTEGER, listing.unit_price ?? Number.MAX_SAFE_INTEGER)
    current.time_left = current.time_left === "SHORT" || listing.time_left === "SHORT" ? "SHORT" : current.time_left
  })
  ...
  const itemSummaryMap = await fetchItemSummaryMap(normalizedListings.map((listing) => listing.item.id))
...
export const fetchConnectedRealmAuctionSnapshots = async (
  ...
  const response = await fetchAuctionHouse(`/data/wow/connected-realm/${connectedRealmId}/auctions`)
  const listings = response.auctions
    .slice(0, MAX_AUCTION_SCAN)
    .filter((listing) => typeof listing.buyout === "number" && listing.buyout > 0)
    .sort((left, right) => (right.buyout ?? 0) - (left.buyout ?? 0))
    .slice(0, limit)

  const itemSummaryMap = await fetchItemSummaryMap(listings.map((listing) => listing.item.id))

  return listings.map((listing) =>
    normalizeAuctionResult(listing, itemSummaryMap.get(listing.item.id), formatCopper(listing.buyout))
  )
}