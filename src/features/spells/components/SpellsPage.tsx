...
const SpellsPage = (): JSX.Element => {
  ...
  const friendlyError = useMemo(() => {
    const error = searchQuery.error ?? detailQueries.find((entry) => entry.error)?.error ?? mediaQueries.find((entry) => entry.error)?.error
    if (!error) {
      return undefined
    }
    ...
    return error instanceof Error ? error.message : "Unable to load spell data right now."
    ...
    <Stack spacing={{ xs: 4, md: 6 }}>
      <Stack spacing={1.5}>
        ...
        <Typography variant="body1" color="text.secondary">
          Search the live spell catalogue and browse the results as icon cards instead of switching between a selector and a detail panel.
        </Typography>
      </Stack>
        ...
        <Stack spacing={2.5}>
          ...
          <Stack direction="row" spacing={1.25} useFlexGap flexWrap="wrap">
            {QUICK_SPELLS.map((spell) => (
              <Button key={spell} variant={query === spell ? "contained" : "outlined"} color="primary" size="small" onClick={() => setQuery(spell)} sx={{ borderRadius: 999 }}>
                {spell}
              </Button>
            ))}
          </Stack>
        ...
        <Stack spacing={3}>
          <Stack spacing={0.5}>
            ...
            <Typography variant="body2" color="text.secondary">
              Showing {gallerySpells.length} live spells.
            </Typography>
