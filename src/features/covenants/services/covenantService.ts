...
const normalizeRenownRewards = (
  entries: Array<{ level: number; reward: { id: number; name: LocalizedString; key: { href: string } } }> = []
): RenownReward[] =>
  [...entries]
    .map((entry) => ({
      level: entry.level,
...
export const fetchCovenantIndex = async (): Promise<CovenantIndexResponse> => {
  const response = await blizzardClient.get<{
    covenants: Array<{ id: number; name: LocalizedString; key: { href: string } }>
...
export const fetchCovenantDetail = async (covenantId: number): Promise<CovenantDetail> => {
  const response = await blizzardClient.get<{
    id: number
  ...
  return {
    ...
    signatureAbility: response.signature_ability ? normalizeAbility(response.signature_ability) : undefined,
    classAbilities: (response.class_abilities ?? []).map(normalizeAbility),
    renownRewards: normalizeRenownRewards(response.renown_rewards),
  }
}

export const fetchCovenantIcon = async (covenantId: number): Promise<string | undefined> => {
  try {
    const response = await blizzardClient.get<CovenantMedia>(`/data/wow/media/covenant/${covenantId}`, {
      namespace: STATIC_NAMESPACE,
    })
