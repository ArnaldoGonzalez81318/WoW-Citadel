...
import { fetchCovenantDetail, fetchCovenantIcon, fetchCovenantIndex } from "@/features/covenants/services/covenantService"
...
const CovenantPage = (): JSX.Element => {
  ...
  const friendlyError = useMemo(() => {
    const error = indexQuery.error ?? detailQueries.find((entry) => entry.error)?.error ?? mediaQueries.find((entry) => entry.error)?.error
    if (!error) {
      return undefined
    }
    ...
    return error instanceof Error ? error.message : "Unable to load covenant data right now."
    ...
    () =>
      covenants.map((covenant, index) => {
        ...
        return {
          ...
          summary: detail?.signatureAbility?.spellTooltip?.spell?.name ?? "Covenant overview",
          details: [
            detail?.description,
            detail ? `${detail.classAbilities.length} class abilities` : undefined,
            detail ? `${detail.renownRewards.length} renown rewards` : undefined,
          ]
            .filter(Boolean)
            .join(" • "),
    ...
    <Stack spacing={{ xs: 4, md: 6 }}>
      <Stack spacing={1.5}>
        ...
        <Typography variant="body1" color="text.secondary">
          Browse covenants as a gallery. Each card is enriched with live covenant details, signature ability data, and media when Blizzard exposes it.
        </Typography>
      </Stack>
        ...
        <Stack spacing={2}>
          {activeEnrichmentCount < covenants.length ? (
            <Typography variant="caption" color="text.secondary">
              Prefetching additional covenant details and media during idle time.
            </Typography>
          ) : null}
          <Grid container spacing={3}>
