...
const ItemsPage = (): JSX.Element => {
  ...
  const [selectedSubclassId, setSelectedSubclassId] = useState<number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const hasSelectedSubclass = subclasses.some(
      (entry) => entry.id === selectedSubclassId,
    );
  ...
  const loadedItems = useMemo(
    () => galleryPages.flatMap((pageEntry) => pageEntry.items),
    [galleryPages],
  );
  ...
  const itemMediaQueries = useQueries({
    queries: loadedItems.map((item) => ({
      ...
      enabled:
        loadedItems.findIndex((entry) => entry.id === item.id) <
        activeMediaCount,
      staleTime: 300000,
      retry: false,
    })),
  ...
  const friendlyError = useMemo(() => {
    ...
    return error instanceof Error
      ? error.message
      : "Unable to load item data right now.";
  ...
  const selectedItemDetailRows = useMemo(() => {
    ...
    return [
      ...
      typeof detail.level === "number"
        ...
        : undefined,
      typeof detail.requiredLevel === "number"
        ? `Required level: ${detail.requiredLevel}`
        : undefined,
      ...
      typeof detail.maxCount === "number" && detail.maxCount > 0
        ? `Max stack: ${detail.maxCount}`
        : undefined,
      formatPrice(detail.purchasePrice)
        ? `Purchase price: ${formatPrice(detail.purchasePrice)}`
        : undefined,
      formatPrice(detail.sellPrice)
        ? `Sell price: ${formatPrice(detail.sellPrice)}`
        : undefined,
  ...
  const selectedItemUsesIconAsset = Boolean(
    selectedItem?.mediaUrl && /\/icons\/56\//.test(selectedItem.mediaUrl),
  );
    ...
    <Stack spacing={{ xs: 4, md: 6 }}>
      <Stack spacing={1.5}>
        ...
        <Typography variant="body1" color="text.secondary">
          Browse live item records as a gallery. Filter by class and subclass,
          then keep scrolling through Blizzard&apos;s item search results while
          the grid fills itself in.
        </Typography>
      </Stack>
          ...
          >
            ...
            >
              ...
              <Select
                ...
                inputProps={{ id: "item-class-select", name: "item-class" }}
            ...
            <Stack direction="row" spacing={1.25} useFlexGap flexWrap="wrap">
              ...
              {subclasses.map((subclass) => (
                <Chip
                  ...
                  clickable
                  color={
                    selectedSubclassId === subclass.id ? "primary" : "default"
                  }
                  variant={
                    selectedSubclassId === subclass.id ? "filled" : "outlined"
                  }
        ...
        <Stack spacing={3}>
          <Stack spacing={0.75}>
            ...
            <Typography variant="body2" color="text.secondary">
              Loaded {galleryItems.length} items across{" "}
              {galleryPages.length || 1} page
              {galleryPages.length === 1 ? "" : "s"} from Blizzard&apos;s live
              item search.
            </Typography>
          </Stack>
          ...
          {galleryItems.length > 0 ? (
            ...
            <Typography variant="body2" color="text.secondary">
              No items matched the current subclass filter. Try another subclass
              or switch back to all subclasses.
            </Typography>
          )}
      ...
      >
        ...
        <DialogContent dividers>
          ...
          >
            <Box
              sx={{
                width: selectedItemUsesIconAsset
                  ? { xs: 56, md: 56 }
                  : { xs: "100%", md: 260 },
                flexShrink: 0,
              }}
            >
              <Box
                sx={{
                  ...
                  minHeight: selectedItemUsesIconAsset
                    ? 56
                    : { xs: 220, md: 260 },
                  display: "flex",
                  alignItems: "center",
                  ...
                  border: selectedItemUsesIconAsset
                    ? "none"
                    : "1px solid rgba(30, 155, 233, 0.16)",
                  backgroundColor: selectedItemUsesIconAsset
                    ? "transparent"
                    : "rgba(7, 12, 24, 0.72)",
            ...
            <Stack spacing={3} sx={{ minWidth: 0, flex: 1 }}>
              ...
              <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
                {selectedItemChipLabels.map((label) => (
                  <Chip
                    ...
                    size="small"
                    color={
                      selectedItemDetailQuery.data?.quality?.toLowerCase() ===
                      label.toLowerCase()
                        ? "primary"
                        : "default"
                    }
              ...
              {selectedItemDetailQuery.data?.description ||
