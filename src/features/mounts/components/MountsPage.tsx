...
const QUICK_MOUNTS = ["Invincible", "Ashes of Al'ar", "Swift Spectral Tiger", "Mimiron's Head"]
const PAGE_SIZE = 24
...
const MountsPage = (): JSX.Element => {
  ...
  const mediaQueries = useQueries({
    queries: detailQueries.map((detailQuery, index) => ({
      queryKey: ["mount-display-media", mounts[index]?.id, detailQuery.data?.displayId, env.region],
      queryFn: () => fetchCreatureDisplayImage(Number(detailQuery.data?.displayId)),
      enabled: index < activeEnrichmentCount && typeof detailQuery.data?.displayId === "number",
      staleTime: 300000,
      retry: false,
    })),
  })
    ...
    () =>
      mounts.map((mount, index) => {
        ...
        return {
          ...
          details: detail?.description || mount.description || "Live mount data pulled from Blizzard's mount catalogue.",
          typeLabel: "Mount",
  ...
  const friendlyError = useMemo(() => {
    ...
    return error instanceof Error ? error.message : "Unable to load mount data right now."
  }, [detailQueries, indexQuery.error, mediaQueries, searchQuery.error])

  const isLoading = indexQuery.isLoading || searchQuery.isLoading
  const hasMoreIndexedMounts = query.trim().length < 2 && mounts.length < indexedMounts.length
    ...
    <Stack spacing={{ xs: 4, md: 6 }}>
      <Stack spacing={1.5}>
        ...
        <Typography variant="body1" color="text.secondary">
          Browse the mount collection as a gallery. The default view now keeps extending as you scroll, while search narrows the grid to matching mounts.
        </Typography>
      </Stack>
        ...
        <Stack spacing={2.5}>
          ...
          <Stack direction="row" spacing={1.25} useFlexGap flexWrap="wrap">
            {QUICK_MOUNTS.map((mount) => (
              <Button key={mount} variant={query === mount ? "contained" : "outlined"} color="primary" size="small" onClick={() => setQuery(mount)} sx={{ borderRadius: 999 }}>
                {mount}
              </Button>
            ))}
            {query ? (
          ...
          <Grid container spacing={2}>
            {Array.from({ length: 12 }).map((_, index) => (
              <Grid item xs={12} md={6} lg={4} key={index}>
                <Skeleton variant="rounded" height={280} sx={{ borderRadius: 3, backgroundColor: "rgba(12, 18, 34, 0.45)" }} />
              </Grid>
            ))}
          </Grid>
        </Stack>
            ...
            <Stack spacing={1.5} alignItems="center">
              ...
              {activeEnrichmentCount < mounts.length ? (
                <Typography variant="caption" color="text.secondary">
                  Prefetching additional mount details and media during idle time.
                </Typography>
              ) : null}
              {hasMoreIndexedMounts ? <Box ref={infiniteScrollRef} sx={{ width: "100%", height: 1 }} /> : null}
            </Stack>
          </Stack>
        </Stack>
      ) : null}
        ...
        <DialogContent dividers>
          <Stack direction={{ xs: "column", md: "row" }} spacing={3} alignItems={{ xs: "stretch", md: "flex-start" }}>
            ...
            <Stack spacing={3} sx={{ minWidth: 0, flex: 1 }}>
              ...
              {(selectedMountDetailQuery.data?.source || selectedMount?.summary) ? (
                <Box>
              ...
              {(selectedMountDetailQuery.data?.description || selectedMount?.details) ? (
                <Box>
                  <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 0.75 }}>
                    Description
                  </Typography>
              ...
              <Link
                href={selectedMountDetailQuery.data?.href ?? selectedMount?.href}
                target="_blank"
                rel="noreferrer"
                color="primary"
                underline="hover"
