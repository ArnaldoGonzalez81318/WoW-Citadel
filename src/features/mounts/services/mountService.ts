...
import { LocalizedString, MountDetail, MountIndexResponse, MountSummary } from "@/features/mounts/types"
...
export const fetchMountIndex = async (): Promise<MountSummary[]> => {
  const response = await blizzardClient.get<MountIndexResponse>("/data/wow/mount/index", {
...
export const fetchMountGalleryPage = async (page: number, pageSize = 24): Promise<MountGalleryPage> => {
  const response = await blizzardClient.get<MountIndexResponse>("/data/wow/mount/index", {
    namespace: STATIC_NAMESPACE,
  })
...
export const searchMountsDetailed = async (query: string, pageSize = 12): Promise<MountSummary[]> => {
  if (!query.trim()) {
    return []
  }

  return safeSearch(async () => {
    const response = await blizzardClient.get<SearchResponse<MountSearchEntry>>("/data/wow/search/mount", {
      namespace: STATIC_NAMESPACE,
      orderby: "id:desc",
      _pageSize: pageSize,
...
export const fetchMountDetail = async (mountId: number): Promise<MountDetail> => {
...
export const fetchCreatureDisplayImage = async (displayId: number): Promise<string | undefined> => {
  try {
    const response = await blizzardClient.get<{
