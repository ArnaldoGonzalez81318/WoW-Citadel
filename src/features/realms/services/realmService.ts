...
import { LocalizedString, RealmDetail, RealmSummary } from "@/features/realms/types"
...
export const fetchRealmIndex = async (): Promise<RealmSummary[]> => {
  const response = await blizzardClient.get<RealmIndexResponse>("/data/wow/realm/index", {
...
export const fetchRealmGalleryPage = async (page: number, pageSize = 24): Promise<RealmGalleryPage> => {
...
export const searchRealmsDetailed = async (query: string, pageSize = 12): Promise<RealmSummary[]> => {
  if (!query.trim()) {
    return []
  }

  return safeSearch(async () => {
    const response = await blizzardClient.get<SearchResponse<RealmSearchEntry>>("/data/wow/search/realm", {
      namespace: DYNAMIC_NAMESPACE,
      _pageSize: pageSize,
...
export const fetchRealmDetail = async (realmSlug: string): Promise<RealmDetail> => {
