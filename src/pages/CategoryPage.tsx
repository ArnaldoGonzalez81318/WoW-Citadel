...
const MountsPage = lazy(
  () => import("@/features/mounts/components/MountsPage"),
);
const RealmsPage = lazy(
  () => import("@/features/realms/components/RealmsPage"),
);
...
const renderLazyPage = (node: ReactNode): ReactNode => (
  <Suspense fallback={<CategoryLoadingFallback />}>{node}</Suspense>
);
...
const findNavItemBySlug = (slug: string): IndexedNavItem | undefined => {
  ...
  return NAV_ITEM_INDEX.find(
    (item) => item.path === pathMatch || item.href === hashMatch,
  );
};
...
const renderContentForSlug = (slug: string): ReactNode => {
  ...
  if (isSearchCategory(slug)) {
    return renderLazyPage(
      <SearchExperience focusCategoryId={slug} variant="category" />,
    );
  }
    ...
    return (
      <Stack spacing={4}>
        ...
        >
          ...
          Data refreshes every 30 minutes using Blizzard's WoW Token commodity
          endpoint. Keep the page open to watch regional trends without
          refreshing the home dashboard.
        </Alert>
      </Stack>
    );
  }
  ...
  >
    <Stack spacing={3}>
      ...
      <Typography variant="body1" color="text.secondary">
        We're building a dedicated experience for {item.label}. Expect curated
        data pulls, filters, and related media once the Blizzard API integration
        lands. In the meantime, use the global search in the header for quick
        lookups.
      </Typography>
...
const CategoryPage = (): JSX.Element => {
  ...
  const navItem = useMemo(
    () => (normalizedSlug ? findNavItemBySlug(normalizedSlug) : undefined),
    [normalizedSlug],
  );
